...
- **A demo session is a signed cookie and nothing else.** No account, no server-side session row: the HMAC covers both the userId and a server-enforced expiry ([src/lib/current-user.ts](src/lib/current-user.ts)). The mint endpoint is POST-only with an Origin check and per-IP plus global rate limits, and abandoned demo accounts are deleted within 14 days by a scheduled sweep ([src/lib/demo-cleanup.ts](src/lib/demo-cleanup.ts)), with the login path keeping the same sweep as a backstop.
