...
The whole product is a single Next.js 16 (App Router) deployable: React UI, JSON API as Route Handlers under [src/app/api](src/app/api), Server Actions for form flows, and Prisma to one Postgres instance. No separate backend, no worker tier, no Redis.
