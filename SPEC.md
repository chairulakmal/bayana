...
3. **Mode grid**: four tiles (Flashcard `/study`, Quiz `/quiz`, Exam `/exam`, Grammar `/grammar`) in a 2×2 layout, each with a subtitle derived from live counts. Grammar is included here because the hub is the app's default page; a mode absent from it is effectively hidden. **No tile is ever disabled.** Each is the sole route to its mode, so a dimmed tile removes a section of the app: with the Grammar nav tab gone, disabling the Grammar tile on levels with no seeded deck (only N3 is seeded, §4.1) made `/grammar` and all existing grammar progress unreachable behind a level switch. Caveats go in the subtitle instead, and `/grammar` itself distinguishes "no deck for this level" from "all caught up" rather than reporting a deck that does not exist as finished.
...
- **`/grammar` hub page:** inline FSRS stats (total/started/mature/due); single "Grammar Points" CTA. Vocab stats remain on `/stats`. Grammar also got a `BottomNav` tab here, removed on 2026-07-25 when the mode grid on `/home` made it redundant (§8.5, §14.8).
