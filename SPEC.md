...
- **Keyboard shortcuts in the four study modes (2026-07-26).** Every mode is fully drivable from the keyboard without Tab. The map is deliberately identical wherever the modes agree, so it transfers between queues in one sitting:
  ...
  | `U` | undo the last rating (Flashcard only; grammar has no undo yet) | not bound |
...
| `rateCard` | `app/study/actions.ts` | Apply an FSRS rating to a word (`reviewWord`) | `getCurrentUserId()` + rating ∈ {1,2,3,4} + non-empty `wordId` | **Implemented** (2026-07-26). `POST /api/review` still stands until the other three modes are ported |
