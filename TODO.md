...
**Then: the UI/UX workstream** (second section). Nine items, of which four are now sequenced behind the architecture work and two remain deferred on external blockers. Each is marked in place.
...
- [ ] `src/app/auth/signin/page.tsx:117`: `useActionState` for the pending state, which resolves the second bullet of UI/UX item 3 as a side effect rather than as separate work.
...
**Items 1 to 7 came from a full UI/UX review of the shipped surface on 2026-07-26**, read against BRAND.md and SPEC §8.4. They cluster on one axis the earlier passes did not cover: what happens on a keyboard, with a screen reader, or in the instant after a tap. The visual system itself held, so nothing below is a token or layout change. The review's eighth item, keyboard shortcuts in the study modes, shipped on 2026-07-26 (SPEC §8.4, §14.18) and is deleted; the renumbering below is from that deletion.

**Items 1, 3, 4 and 7 now sequence behind the architecture workstream above**, which rewrites the same lines. Marked in place. Items 2, 5 and 6 are independent and can be pulled at any time.

**Items 8 and 9 are deferred, and neither is waiting on capacity.** One is blocked on other work, the other on a decision that was explicitly postponed. Do not pull either in as filler.
...
### 2. The flashcard is one giant `<button>`
...
### 3. The magic-link flow leaves the brand at its most fragile moment
...
### 4. Missing headings and page titles
...
### 5. Labelling and live-region gaps
...
### 6. `grammar-browse-client.tsx` never got its sibling's fixes
...
### 8. Performance: subset the Japanese face, blocked on bayan
...
### 9. UX correctness: day boundaries, deferred pending a timezone decision
