...
**Then: the UI/UX workstream** (second section). Ten items, of which four are now sequenced behind the architecture work and two remain deferred on external blockers. Each is marked in place.
...
- [ ] `src/app/auth/signin/page.tsx:117`: `useActionState` for the pending state, which resolves the second bullet of UI/UX item 4 as a side effect rather than as separate work.
...
**Items 1 to 8 came from a full UI/UX review of the shipped surface on 2026-07-26**, read against BRAND.md and SPEC §8.4. They cluster on one axis the earlier passes did not cover: what happens on a keyboard, with a screen reader, or in the instant after a tap. The visual system itself held, so nothing below is a token or layout change.

**Items 1, 4, 5 and 8 now sequence behind the architecture workstream above**, which rewrites the same lines. Marked in place. Items 2, 3, 6 and 7 are independent and can be pulled at any time.

**Items 9 and 10 are deferred, and neither is waiting on capacity.** One is blocked on other work, the other on a decision that was explicitly postponed. Do not pull either in as filler.

### 1. Focus is destroyed after every answer, in all four modes

The highest-frequency defect in the app: a keyboard user re-Tabs from the top of the document on every card, ten to twenty times a session.
...
### 2. No keyboard shortcuts in any study mode

- [ ] No space-to-flip, no `1` to `4` to rate, no digit keys for MC options, no Enter to continue. The only `keydown` listeners in the codebase are the two Escape handlers in `user-menu.tsx` and `info-bubble.tsx`.
- [ ] **Decide before coding**: BRAND.md makes desktop additive, so this is a feature rather than a defect, and it is the one item here that could legitimately be declined. Weigh it against the fact that the handler it needs also resolves most of item 1. If it ships, record the key map in SPEC §8 and a DECISIONS.md row.

### 3. The flashcard is one giant `<button>`

`src/components/study-session.tsx:293` and `src/components/grammar-session.tsx:239` wrap the whole card, revealed content included, in a single button.

- [ ] Blink and WebKit apply `user-select: none` to button content, so a learner cannot select or copy the word or the example sentence into a dictionary. On a vocabulary app that is a functional loss, not a polish item.
- [ ] A screen reader announces the entire flipped card (expression, reading, meaning, JP sentence, its reading, English) as one button name.
- [ ] Once flipped, the element is an inert focusable button that does nothing.
- [ ] Likely shape: keep tap-to-flip as an overlay button that unmounts on flip, leaving the existing "Show answer" button as the real accessible control.

### 4. The magic-link flow leaves the brand at its most fragile moment
...
### 5. Missing headings and page titles
...
### 6. Labelling and live-region gaps
...
### 7. `grammar-browse-client.tsx` never got its sibling's fixes
...
- [ ] Grammar has no Undo while vocab does (`grammar-session.tsx:8` calls it a v1 omission). A mis-tapped "Easy" on a grammar card is as unrecoverable as the last-card vocab case that was worth fixing in `study-session.tsx`. **Cheapest during architecture item 3**, which is already writing that component's action layer.
...
### 9. Performance: subset the Japanese face, blocked on bayan
...
### 10. UX correctness: day boundaries, deferred pending a timezone decision
